...
print(chi2_test) #p-value equals 0.02951, this is lower than 0.05 so these tell 
...
set.seed(123) #for reproducibility
# more detailed dendrogram, used in the document, helps us understand better
fviz_dend(hc,
          k = 3,               
          cex = 0.7,           
          lwd = 0.5,           
...
# we cut the tree in three groups
grupos <- cutree(hc, k = 3)

# we add those groups to the original dataframe to see the means
df_final$cluster_jerarquico <- as.factor(grupos)

# we can see what characterises each group
...
cols_hc <- c("#00AFBB", "#E7B800", "#2E9FDF") #these are the same as in the dendrogram, for visual cohesion
